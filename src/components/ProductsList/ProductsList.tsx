...
export const ProductsList: React.FC<Props> = ({ products }) => {
  ...
  const title =
    categoriesList.find(c => `/${c.name}` === pathname)?.title ||
  ...
  return (
    <>
        ...
        <div className="Products grid">
          <div className="Products__top">
            ...
            <p className="Products__text">
              {query && <span>{`${sortedProducts.length} results`}</span>}

              {!query && !title.includes('Favourites') && (
                <span>{`${sortedProducts.length} models`}</span>
              )}
          ...
          {!query && !title.includes('Favourites') && (
            <div className="Products__selects">
              <div className="Products__select-wrapper">
                <label htmlFor="sort" className="Products__select-lable">
                  Sort by
                </label>
              ...
              <div className="Products__select-wrapper">
                <label htmlFor="pagination" className="Products__select-lable">
                  Items on page
                </label>
      ...
      {products.length === 0 && <NoResults categoryName={title} />}
    </>
  );
};

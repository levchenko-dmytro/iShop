  ...
  const handleAdd = (itemId: string) => {
    ...
    if (updated) {
      if (cartItems.some(item => item.id === itemId)) {
        ...
        setCartItems([
          ...cartItems,
          { id: updated.itemId, quantity: 1, product: updated },
        ]);
      }
    }
  };
  ...
  const handleFavourite = (itemId: string) => {
    ...
    if (updated) {
      if (favouriteItems.some(item => item.itemId === itemId)) {
        setFavouriteItems(
          [...favouriteItems].filter(item => item.itemId !== itemId),
        );
      } else {
  ...
  return (
    <div className="Buttons">
      <button
        ...
        className={classNames('Buttons__button', 'Buttons__button--add', {
      ...
      <button
        ...
        className={classNames('Buttons__button', 'Buttons__button--favourite', {
          'active-button': favouriteIds.includes(id),
        })}
